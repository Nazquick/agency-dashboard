...
import { REPORT_LABELS, type ReportLanguage } from "@/lib/reports/i18n";
...
export type ReportData = {
  ...
  stats: {
    ...
    topRole: { role: string; doneCount: number } | null;
  };
  tasks: {
    title: string;
    status: string;
  ...
  bestAsset: { title: string; platform: string } | null;
  worstAsset: { title: string; platform: string } | null;
  salesCampaign: {
...
const styles = StyleSheet.create({
  ...
  cornerTR: { position: "absolute", top: 16, right: 20 },
  cornerBL: { position: "absolute", bottom: 16, left: 20 },
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>{t.subtitle(new Date(generatedAt).toLocaleDateString())}</Text>
        ...
        <View>
          ...
          <View style={styles.statGrid}>
            ...
            <View style={styles.statBox}>
              ...
              <Text style={styles.statLabel}>{t.estimatedHours}</Text>
            </View>
            <View style={styles.statBox}>
              <Text style={styles.statValue}>{stats.roleActivity.length}</Text>
              <Text style={styles.statLabel}>{t.rolesActivatedStat}</Text>
            </View>
          </View>
        </View>
        ...
        {sections.rolesSummary && stats.roleActivity.length > 0 && (
          <View>
            <Text style={styles.sectionTitle}>{t.rolesActivatedSection}</Text>
            {stats.topRole && (
              <Text style={[styles.paragraph, { marginBottom: 8 }]}>
                {t.contributedMostPrefix}
                <Text style={{ fontWeight: 700 }}>{stats.topRole.role}</Text>{" "}
                {t.completedTasksSuffix(stats.topRole.doneCount)}
              </Text>
            )}
            <View style={styles.tableHeaderRow}>
              <Text style={[styles.th, col("70%")]}>{t.roleCol}</Text>
              <Text style={[styles.th, col("30%")]}>{t.tasksCol}</Text>
            </View>
        ...
        {sections.taskList && tasks.length > 0 && (
          <View>
            <Text style={styles.sectionTitle}>{t.workPerformed}</Text>
            <View style={styles.tableHeaderRow}>
              <Text style={[styles.th, col("40%")]}>{t.taskCol}</Text>
              <Text style={[styles.th, col("20%")]}>{t.roleCol}</Text>
              <Text style={[styles.th, col("15%")]}>{t.statusCol}</Text>
              <Text style={[styles.th, col("15%")]}>{t.priorityCol}</Text>
              <Text style={[styles.th, col("10%")]}>{t.hoursCol}</Text>
            </View>
            {tasks.map((task, i) => (
              <View style={styles.tableRow} key={i}>
                <Text style={[styles.td, col("40%")]}>{task.title}</Text>
                <Text style={[styles.td, col("20%")]}>{task.assigneeRole}</Text>
                <Text style={[styles.td, col("15%")]}>{task.status}</Text>
                <Text style={[styles.td, col("15%")]}>{task.priority}</Text>
                <Text style={[styles.td, col("10%")]}>{task.hours ? task.hours.toFixed(1) : "—"}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {sections.engagement && assets.length > 0 && (
          <View>
            <Text style={styles.sectionTitle}>{t.contentEngagement}</Text>
            <View style={styles.tableHeaderRow}>
              <Text style={[styles.th, col("34%")]}>{t.assetCol}</Text>
              <Text style={[styles.th, col("16%")]}>{t.viewsCol}</Text>
              <Text style={[styles.th, col("16%")]}>{t.likesCol}</Text>
              <Text style={[styles.th, col("17%")]}>{t.commentsCol}</Text>
              <Text style={[styles.th, col("17%")]}>{t.sharesCol}</Text>
            </View>
            {assets.map((a, i) => (
              <View style={styles.tableRow} key={i}>
        ...
        {sections.performance && assets.length > 0 && (
          <View>
            ...
            <View style={styles.statGrid}>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{bestAsset?.title ?? "—"}</Text>
                <Text style={styles.statLabel}>
                  {t.bestAsset}
                  {bestAsset ? ` (${bestAsset.platform})` : ""}
                </Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{worstAsset?.title ?? "—"}</Text>
                <Text style={styles.statLabel}>
                  {t.worstAsset}
                  {worstAsset ? ` (${worstAsset.platform})` : ""}
                </Text>
              </View>
            </View>
            <Text style={[styles.paragraph, { marginTop: 10 }]}>
              {t.topPerformers}
              {assets.filter((a) => a.tier === "high").map((a) => a.title).join(", ") || "—"}
            </Text>
            <Text style={styles.paragraph}>
              {t.needsAttention}
              {assets.filter((a) => a.tier === "low").map((a) => a.title).join(", ") || "—"}
            </Text>
          </View>
        )}

        {sections.salesCampaign && (
          <View>
            <Text style={styles.sectionTitle}>{t.salesCampaign}</Text>
            {salesCampaign.reportDate && (
              <Text style={[styles.paragraph, { marginBottom: 8 }]}>
                {t.campaignFiguresFrom(new Date(salesCampaign.reportDate).toLocaleDateString())}
              </Text>
            )}
            <View style={styles.statGrid}>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>
                  {salesCampaign.salesGrowthPercent != null
                    ? `${salesCampaign.salesGrowthPercent > 0 ? "+" : ""}${salesCampaign.salesGrowthPercent}%`
                    : "—"}
                </Text>
                <Text style={styles.statLabel}>{t.salesGrowth}</Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{salesCampaign.roas ?? "—"}</Text>
                <Text style={styles.statLabel}>{t.roas}</Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>
                  {salesCampaign.adSpend != null ? fmtKr(salesCampaign.adSpend) : "—"}
                </Text>
                <Text style={styles.statLabel}>{t.adSpend}</Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>
                  {salesCampaign.appDownloads?.toLocaleString() ?? "—"}
                </Text>
                <Text style={styles.statLabel}>{t.appDownloads}</Text>
              </View>
            </View>
            <View style={[styles.statGrid, { marginTop: 10 }]}>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{fmtKr(salesCampaign.valueSold)}</Text>
                <Text style={styles.statLabel}>{t.soldValue}</Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{salesCampaign.unitsSold.toLocaleString()}</Text>
                <Text style={styles.statLabel}>{t.soldUnits}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        {sections.advice && (
          <View>
            <Text style={styles.sectionTitle}>{t.adviceTitle}</Text>
            {advice.length > 0 ? (
              advice.map((tip, i) => (
                <Text style={[styles.paragraph, { marginBottom: 6 }]} key={i}>
                  {i + 1}. {tip}
                </Text>
              ))
            ) : (
              <Text style={styles.paragraph}>{t.adviceEmpty}</Text>
            )}
          </View>
        )}
