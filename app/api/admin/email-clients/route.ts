...
export async function POST(request: Request) {
  ...
  const { data: contacts } = await supabase
    ...
    .select("client_id, email, is_primary")
    .not("email", "is", null)
  ...
  for (const contact of contacts ?? []) {
    ...
    if (!emailByClient.has(contact.client_id)) {
      emailByClient.set(contact.client_id, contact.email);
    }
  }
  const recipients = [...new Set(emailByClient.values())];
